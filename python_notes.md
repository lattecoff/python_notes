...
bot.
Если хорошо мы получим **ключ** который **нельзя разглашать**
...
executor.start_polling(dp, skip_updates = True) # когда True, бот будет пропускать сообщения которые поступали когда он был не онлайн.
```
